...
def main():
    ...
    train_input, train_label, valid_input, valid_label = unpack_dataset(
        DatasetInput, DatasetLabel, IdxValid)
    print("Train data shape:", train_input.shape, train_label.shape)
    print("Valid data shape:", valid_input.shape, valid_label.shape)

    ResultDir = "result/subject_%s/" % (to)
    ...
    model = Network.SolveNet(n_input=n_input, n_output=n_label).to(device)
    ...
    for idx_epoch in range(EPOCH+1):
        ...
        if idx_epoch == 0 or (idx_epoch+1) % SavePeriod == 0:
            ...
            save_result(valid_pred, idx_epoch+1, ResultDir)
