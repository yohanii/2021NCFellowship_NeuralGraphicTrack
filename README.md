...
**1인 개발 / Pytorch 1.9.0, Python 3.6.8, Google Colab**

### Summary

---

**노이즈가 포함된 마커 데이터에서 머신러닝을 통해 부드럽고 깨끗한 스켈레톤 애니메이션을 추정하는 프로젝트**입니다. NCsoft에서 주관하는 ‘**NC Fellowship Neural Graphic Track**’대회에서 진행한 프로젝트로 5개월 동안 스스로 공부를 하고 남은 2개월 동안 배운 내용을 토대로 머신러닝 모델을 만드는 과정으로 이루어졌습니다.

**입력 데이터**는 CMU Graphics Lab Motion Capture Database에서 추출한 마커들의 Position 데이터이고, **출력 데이터**는 BVH에서 추출한 관절들의 Euler angle 데이터에서 변환된 Rotation Matrix 데이터입니다.

![목표](https://user-images.githubusercontent.com/33834623/169675607-fbab51a5-bf79-4542-a071-367a0cdd411e.png)



![초기](https://user-images.githubusercontent.com/33834623/169675611-2f986565-3afc-474c-bed2-3f4654dd74dc.gif)

초기


![결과](https://user-images.githubusercontent.com/33834623/169675602-349118f6-2867-4afc-a9a8-c82dea598a0b.gif)
...
1. VGG, RestNet 적용 → 결과 좋지 않음

→ 무작정 네트워크를 적용하지 말고, dataset에 대한 이해를 기반으로 적용시켜야겠다고 생각함.

1. “Robust solving of optical motion capture data by denoising” 논문을 읽고, Dense layer와 ReLU로 이루어진 Residual block을 5개 이은 네트워크를 채택함.  

→ Dense layer로도 충분히 학습이 잘 될것이라 판단했고, loss가 수렴하는 것으로 보아 학습은 잘 되지만, 전처리가 부족한 것이 문제로 보임.

1. 입력데이터의 41개의 노드 중 관절 부위의 17개의 노드만 추출해서 로컬 좌표와 몸 쪽 6개의 노드로 Root 좌표(글로벌 좌표)를 추출해 따로 학습 시킴. (입력데이터 단순화)

→ 많이 개선됨.

1. L2 loss에서 L1 loss로 바꿈

→ 가장 효과가 좋았음.

1. 마커데이터에서 키를 일정하게 맞추는 식으로 스케일링 적용해봄.

→ smoothness는 개선되었지만, valid_loss는 개선되지 않음.

1. 쿼터니안을 사용해 output이 rotation matrix로 9개 나오던 것을 4개로 줄임. (아웃풋 단순화)

→ 많이 버벅거리고, 노이즈에 약한 모습을 보임.

1. 마커데이터의 축이 잘못되어있는 것을 확인해서 (x, y, z) 축을 (y, z, x) 축으로 바꿔줌.

→ 어느 정도 효과 있다.

1. 노이즈를 제거하기 위해 연속된 프레임에서 일정 거리 이상 멀어지는 노드를 찾아서 이전 프레임의 좌표로 바꿔줌.

→ 큰 효과 없음. 알맞은 거리를 선택하는 것이 어떤 거리를 선택하던 문제가 발생함. 

결과적으로 2, 3, 4, 7을 적용한 모델이 최종본으로 선택됩니다. 
...
시간을 많이 투자해 시도한 방법들이 생각보다 효과가 좋지 않아서 아쉬운 부분이 있습니다. 시도한 방법 중에 Loss function을 L1 loss로 바꾼 것이 가장 효과가 좋았는데, loss function에 시간을 더 투자했으면 좋았을 것 같습니다. 

이번 대회를 통해 **작지만 많은 실패를 경험한 점**이 가장 좋았습니다. 프로젝트를 진행하면서 하루에도 여러 번 실패를 경험하는 것이 일상이었는데, 특히 무조건 성과가 있을 것이라고 확신했을 때 일어나는 실패는 더 아프게 느껴졌습니다. 그래도 이런 고통이 저를 성장시켜줄 것이라고 믿으며 계속 실패와 재도전을 반복하다 보니 어느새 끝까지 올 수 있었던 것 같습니다. 

**쉽게 포기하지 않는 법**을 배웠습니다. 팀원이 그만둔 순간 저도 같이 포기했다면, 이 정도의 성장은 절대 불가능했을 것입니다. 그간 해본 토이 프로젝트 중에서는 끝까지 해본 경험이 적었는데, 이번에 끝까지 해보니 정말 뿌듯하고 다음엔 다른 프로젝트를 더 열심히 해보고 싶다는 생각도 들었습니다.
